...
- Lock brand tokens via brand-consistency skill for every brand-facing prompt
...
**REQUIRED SKILL:** Use brand-consistency skill to lock brand tokens.
...
1. **Receive visual direction from The Art Director** — The visual direction document includes mood, color palette, lighting, composition, style, and model recommendation.
2. **Apply image-prompt-engineering skill:**
   ...
   d. Add brand tokens (via brand-consistency skill) — inject locked brand terms, colors, and prohibited-element exclusions
