...
- `wispr-flow-v2/`: An example implementation of a design system generated by this agent.
...
Copy the `AGENT.md` instructions into your project's agent configuration:

```bash
mkdir -p .opencode/agents
cp recipes/workflows/the-designer/AGENT.md .opencode/agents/the-designer.md
```
...
1. **Start a session**: Mention the agent using `@the-designer`.
2. **Provide a prompt**: 
