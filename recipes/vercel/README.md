...
```sh
/lab install the-librarian
/lab install vercel
```

Auto-install of required recipes by `/lab install` is a planned follow-up. For
now, install `the-librarian` before you install `vercel`.
...
of this lab. Use `/lab install vercel` when the lab CLI is installed.
