...
**Elevation**: A string representing a CSS `box-shadow` value (e.g., `"0 2px 8px 0 rgba(0,0,0,0.08)"`).

**Border**: A map with the following properties:
- `width` (Dimension) - The border width (e.g., `1px`, `0.0625rem`)
- `style` (string) - The border style keyword (e.g., `solid`, `dashed`, `dotted`)
- `color` (Color | token reference) - The border color
...
The `scales` section defines the mathematical formulas that generate size values for typography, spacing, and rounded corners. Instead of hardcoding each size, you declare a scale — the agent computes every size token from it.

**Scale** is a map with the following parameters:

```yaml
scales:
  typography:
    base: <Dimension>   # step 0 size (xs)
    ratio: <number>     # multiplier per step (e.g., 1.25, 1.333, 1.618)
  spacing:
    base: <Dimension>   # step 0 size (xs)
    factor: <number>    # multiplier per step (e.g., 1.5, 2)
```

### Step Levels

Every scale uses the same implicit step levels, ordered from smallest to largest:

`2xs`, `xs`, `sm`, `md`, `lg`, `xl`, `xxl`, `xxxl`

Step 0 = `xs` (the base). Positive indices go larger, negative indices go smaller.

### Formulas

The value for a given step level is computed as:

| Scale | Formula | Example (typography: base=0.75rem, ratio=1.25) |
|-------|---------|-------------------------------------------------|
| `typography` | `base × ratio^stepIndex` | `md` (step 2) = `0.75rem × 1.25² = 1.172rem` |
| `spacing` | `base × factor^stepIndex` | `lg` (step 3) = `0.25rem × 2³ = 2rem` |
| `radius` | `base × factor^stepIndex` | `sm` (step 1) = `0.25rem × 2¹ = 0.5rem` |

The `radius` scale has a special step: `full` always resolves to `9999px` regardless of the scale parameters.

### Computation table

Given `typography: { base: 0.75rem, ratio: 1.25 }`:

| Step | Index | Value   | Common usage          |
|------|-------|---------|-----------------------|
| 2xs  | -2    | 0.48rem | micro labels          |
| xs   | 0     | 0.75rem | captions              |
| sm   | 1     | 0.938rem | small body           |
| md   | 2     | 1.172rem | body text             |
| lg   | 3     | 1.465rem | h4/h5                 |
| xl   | 4     | 1.831rem | h3                    |
| xxl  | 5     | 2.289rem | h1/h2                 |
| xxxl | 6     | 2.861rem | display               |

### Token naming convention

All size tokens generated from a scale follow the pattern `{category}-{step}`:

| Category       | Token examples                       | CSS variable                  |
|----------------|--------------------------------------|-------------------------------|
| `font-size`    | `font-size-xs`, `font-size-md`       | `--font-size-xs`              |
| `spacing`      | `spacing-sm`, `spacing-xl`           | `--spacing-xl`                |
| `radius`       | `radius-md`, `radius-lg`, `radius-full` | `--radius-full`           |

When `scales` is defined in the frontmatter, the agent:
1. Computes each step value using the formula
2. Generates CSS variables using the `--{category}-{step}` naming pattern
3. Updates `spacing` and `rounded` token sections with computed values, and generates the `font-size` token group from `scales.typography`

When `scales` is absent, the design system falls back to fully explicit token values (backward-compatible).
...
This section defines typography **usage tokens** — semantic roles that describe how text is used, separate from their size.

A common naming convention is to use semantic categories such as `body`, `heading`, `label`, `code`, `caption`. Usage tokens define font styling (family, weight, line-height, letter-spacing) but do **not** include size information. Font sizes are defined separately by the `font-size` token group, computed from `scales.typography`. Components combine a usage token with a font-size step.
...
- **Body:** Public Sans Regular ensures contemporary professionalism
...
The `typography` section defines named font usage tokens. Each token describes the **font styling** (family, weight, line-height, letter-spacing) for a semantic role — it does **not** include any size information. Font sizes are defined separately via the `font-size` token group (computed from `scales.typography`).

It is a
map\<string, Typography>

Token names follow a **usage-based** convention — e.g., `typography.body`, `typography.heading`, `typography.label`, `typography.code`, `typography.caption`.

The Typography type **does not** include a `fontSize` property. Font size is chosen per component by referencing the desired step from the `font-size` token group (e.g., `fontSize: "{font-size.md}"`).
...
The `elevation` section defines named `box-shadow` presets. Each preset is a full CSS `box-shadow` value. A common convention is 3 levels: `subtle`, `medium`, `deep`. The token values are referenced in the `components` section via the `{elevation.<preset-name>}` syntax.
...
This section describes how visual elements are shaped. The shape language includes both corner radii (defined by `rounded` tokens) and border styling (defined by `border` tokens).

### Design Tokens

The `border` section defines named border presets. Each preset is a map with `width`, `style`, and `color` properties. A common convention is 4 levels: `none`, `hairline`, `thin`, `thick`.
...
- textColor: \<Color\>
- typography: \<Typography\> — usage token reference (e.g., `{typography.body}`) — provides fontFamily, fontWeight, lineHeight, letterSpacing
- fontSize: \<token reference\> — references a font-size scale step (e.g., `{font-size.md}`)
- rounded: \<Dimension\>
- padding: \<Dimension\>
- elevation: \<token reference\> — references an `elevation` preset (e.g., `{elevation.soft}`)
- border: \<token reference\> — references a `border` preset (e.g., `{border.hairline}`)
- size: \<Dimension\>
- height: \<Dimension\>
- width: \<Dimension\>
...
When using scale-based tokens, the naming convention is `{category}-{step}`. When defining tokens explicitly (without scales), any descriptive name is valid.

**Scale-based naming:**

| Category | Token pattern | Examples |
|----------|---------------|----------|
| `font-size` | `font-size-{step}` | `font-size-xs`, `font-size-md`, `font-size-xxl` |
| `spacing` | `spacing-{step}` | `spacing-xs`, `spacing-lg`, `spacing-xxxl` |
| `radius` | `radius-{step}` | `radius-sm`, `radius-md`, `radius-full` |

**Typography usages:** `body`, `heading`, `label`, `code`, `caption`

**Colors:** `primary`, `secondary`, `tertiary`, `neutral`, `surface`, `on-surface`, `error`

**Rounded (explicit):** `none`, `sm`, `md`, `lg`, `xl`, `full`

**Elevation:** `subtle`, `medium`, `deep`
