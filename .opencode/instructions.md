...
**Marketing Studio**: Use the `the-marketing-studio` agent chain when the task involves AI image generation for marketing content. Agents: Strategist → Art Director → Prompt Engineer. Final image generation is handled by the `image-studio` MCP server (use `image-studio` tool `generate_image`).
